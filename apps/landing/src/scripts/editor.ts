...
import { templateBpmnPath } from "./template-handoff.js"
...
// ── Open a gallery template (/editor?template=<id>) ───────────────────────────
async function openGalleryTemplate(id: string): Promise<void> {
	const path = templateBpmnPath(id)
	const res = path === null ? null : await fetch(path).catch(() => null)
	if (res?.ok !== true) {
		await showConfirmDialog({
			title: "Template not found",
			message: `There is no template "${id}". Browse the gallery at /templates.`,
			confirmLabel: "OK",
		})
		return
	}
	bridge.tabsPlugin.api.openTab({ type: "bpmn", xml: await res.text(), name: `${id}.bpmn` })
	// Drop the parameter so a reload does not open a second copy.
...
requestAnimationFrame(() => {
	...
	if (templateId !== null) {
		void openGalleryTemplate(templateId)
