...
- d910fae: `createCompactStream()` reads a diagram out of a model's token stream, so it can be rendered while it is still being written.
...
- 8fdc6d4: Builder ids are derived from the model instead of generated at random, so
