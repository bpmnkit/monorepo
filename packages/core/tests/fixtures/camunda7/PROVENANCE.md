...
| `claim-handling.bpmn` | constructs the parser does not keep in the model: `camunda:collection` on a multi-instance loop and an external-task implementation on a message end event, recovered from the source XML; plus field injection, `camunda:list`/`camunda:map`, templated and scripted parameters, a Camunda 7 connector and an embedded form key |
