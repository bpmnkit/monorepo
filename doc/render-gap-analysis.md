...
#### P2-2 · Rules engine — **M** — ✅ DONE (2026-07-06)
`rules.ts` is now a small `BpmnRules`-style predicate set: `canConnect` (end/start-event flow direction, boundary events are never a flow target, data/annotation elements use associations not sequence flows, event-based gateway → catch-event/receive-task only), `canAttach` (boundary events attach to activities only), `canContain` (containers hold flow nodes; no nested pools), `canResize` (backs `RESIZABLE_TYPES`), and `canMorph` (same-category only, via `morphCategory`). Wired: `_doConnect` refuses illegal sequence flows at commit **and** the connect drag now shows a red "invalid" ghost line over a forbidden/self target (`_isConnectTargetInvalid` → `setGhostConnection(wps, invalid)` + `.bpmnkit-ghost-conn-invalid` CSS, `previewConnect` gained the hovered `targetId`); boundary-host detection uses `canAttach`; resize handles use `canResize`; `changeElementType` guards with `canMorph`. The editor's duplicated `ACTIVITY_TYPES`/`isActivityType` were removed in favour of the rules module. Tests: `rules` (positive+negative per predicate) + `editor` morph-guard wiring. **Deferred (noted):** auto-morph of a cross-pool sequence flow into a message flow (bpmn-js `ReplaceConnectionBehavior`) and active `canContain` enforcement on drop — the editor has no reparent-on-drop yet, so `canContain` ships as a tested predicate for that future work.
...
- **P3-1 i18n (S/M):** ✅ DONE (2026-07-06) — new `packages/editor/src/i18n.ts` exports the `Translate` type + `defaultTranslate`/`interpolate` (identity + `{name}` placeholder filling). `EditorOptions.translate` is resolved in the constructor and exposed via `editor.translate`; the HUD reads it and localizes palette group titles + item tooltips, the group "(hold for options)"/"(click to change)" affordances, the simulation banner, the search bar, the keyboard-shortcuts panel, and context-menu actions (centralized in `makeCtxItem`). English strings are the keys, so unlocalized keys pass through unchanged. Tests: `i18n` (identity + interpolation, `editor.translate` option, German palette-tooltip localization). **Deferred (noted):** remaining action-bar/config-toolbar tooltips (link process/form/decision) are mechanical follow-ups; the zero-dependency `@bpmnkit/canvas` has no user-facing strings so `translate` lives on `EditorOptions` only.
- **P3-2 Touch editing (M):** ✅ DONE (behavior) (2026-07-06) — a differentiator (bpmn.io removed touch in 2024). Three pieces landed: (1) a `@media (pointer: coarse)` block enlarges drag targets to finger size — resize handles grow to 24px (re-centred with a transform) and endpoint/waypoint/port circles to `r: 12px`; (2) a **long-press** (touch pointer held 500ms without moving past a 10px slop) opens the context menu by dispatching a synthetic `contextmenu` at the press point, reusing the existing handler — cancelled on move/up/cancel; (3) a **double-tap** on a shape (two touch pointer-ups < 300ms, < 20px apart, same id) starts label editing (mirrors dblclick), via `_detectDoubleTap`. Tests: `touch editing` (coarse-pointer CSS present, long-press opens menu + move cancels it, double-tap starts label edit) driven by synthetic touch pointer events. **Deferred (noted):** the AC's real-device **Playwright touch-emulation smoke test** — the repo has no Playwright harness (`@playwright/test` isn't a dependency); the behavior is covered by deterministic happy-dom unit tests instead.
- **P3-3 Accessibility deepening (M):** ✅ DONE (2026-07-06) — the editor mounts a visually-hidden `aria-live="polite"` region (the SVG is `aria-hidden`, so this is the AT channel) and announces selection (`"<name> selected"` / `"<n> elements selected"`) and edit results via the command label (`_executeCommand` announces `t(label)` — Delete/Move/Resize/Connect/Paste/Align…); a cleared selection is intentionally silent so an edit's own result isn't clobbered. A distinct keyboard-focus ring (`.bpmnkit-canvas-host:focus-visible`, dashed `accent-bright`) is visually separate from the solid selection outline. The canvas renderer adds `aria-expanded` to expandable containers (sub-process variants reflect `isExpanded`; call activity is always `false`). Announcements pass through the P3-1 `translate` hook. New `doc/accessibility.md` statement. Tests: editor `accessibility` (live region, selection/edit announcements, translate routing) + canvas `aria-expanded`. **Deferred (noted in the a11y doc):** per-element keyboard traversal of the editor canvas (the editor drives AT via the live region rather than exposing SVG roles).
...
- **P3-6 Replace-menu completeness (M):** ✅ DONE (2026-07-06) — the typed-event morphs and grouped replace entries (`ELEMENT_GROUPS` → group picker) already existed; the real gap was **morph fidelity**: `changeElementType` rebuilt elements from a `base` that dropped `documentation`, `isForCompensation`, and `loopCharacteristics`, so morphing a task that the config panel had given a multi-instance loop silently destroyed it. `base` now carries `documentation`/`isForCompensation`, and after the switch the multi-instance/loop marker is re-grafted whenever the target is an activity type that supports one (`ACTIVITY_LOOP_TYPES`). Collapsed/expanded state rides on the DI shape (untouched by a type change), so it is preserved automatically. Tests: `changeElementType` preserves loop + documentation across an activity morph and drops the loop when morphing to a gateway. **Deferred (noted):** MI *editing* is owned by the `config-panel-bpmn` plugin (not duplicated as a replace-menu toggle); a true collapse/expand that relocates children between planes is P0-1 plane surgery, out of scope here.
