...
**Fonts are self-hosted** — `apps/landing/public/fonts`, ~140 KB across six woff2 subsets, served from the site's own origin so the first paint needs no third-party request and no `fonts.googleapis.com` round trip precedes it. Space Grotesk is one variable file per subset covering the whole 300–700 range, so all four weights the design uses cost one download; Space Mono is static, 400 and 700. `latin` and `latin-ext` are carried with the upstream `unicode-range` values, which is what keeps a page that never leaves ASCII from fetching the extended subsets — in practice no page on the site does, so only three files are ever requested. `font-display: swap` keeps text visible while they arrive. Both families are OFL 1.1; `OFL-SpaceGrotesk.txt`, `OFL-SpaceMono.txt` and a `README.md` recording provenance and the refresh procedure sit alongside the files.
...
- **WP0**: fixed `scripts/update-connectors.mjs`'s stale output path (now writes `packages/connectors/src/templates/generated.ts` + a `catalog-meta.json` sidecar); corrected `plugins-claude/bpmnkit-claude/skills/deploy/SKILL.md` and `skills/worker/SKILL.md` to match the real `bpmn_deploy`/`@bpmnkit/worker-client` APIs.
- **WP1**: new published package `@bpmnkit/connectors` — owns the 116-template OOTB catalog (moved from `packages/plugins`), `listConnectors()`/`searchConnectors()`/`getTemplate()`, and `applyElementTemplate()`/`applyConnectorTemplate()` (full binding-kind coverage: input/output/taskHeader/taskDefinition/property/adHoc, dropdown-gated conditions, FEEL validation). `packages/plugins` now re-exports from it.
- **WP2**: `packages/core/src/bpmn/agentic.ts` — `buildAiAgentSubProcess()`, a deterministic constructor for Camunda's AI Agent Sub-process (tools as root-node activities, `fromAi()` schema generation, verified against the real bundled job-worker template's binding keys). Builder additions: `documentation`, `zeebe:properties`, ad-hoc `completionCondition`/`cancelRemainingInstances`, message `correlationKey`, user-task assignment/schedule/priority.
- **WP3**: `packages/core/src/plan/` — a `ProcessPlan` JSON IR plus `compilePlan()`/`extractPlan()`/`mergePlan()`, driving the 3 fluent builder classes uniformly. New CLI: `casen synth`, `casen plan extract|schema`, `casen connector search|list|show`.
- **WP4**: new optimizer categories `feel-syntax` (parse-validates every FEEL expression in the tree), `deploy` (Zeebe/Reebe deploy-parity rules) and `agentic` (AI Agent sub-process structural rules), plus `connector/missing-required` via an injected resolver. `casen lint --profile deploy` is the deploy-readiness gate.
- **WP5**: `packages/engine`'s dispatcher now routes job-worker-backed ad-hoc sub-processes (e.g. the AI Agent Sub-process) through the existing job-mock mechanism instead of silently auto-completing, so scenarios can mock them like any other task. `bpmn_simulate` (`apps/proxy`) is now honest about what it does: with `scenarios`, it actually executes them via `@bpmnkit/engine`'s `runScenario` (`mode: "execution"`); without, it stays structural-only (`mode: "structural"`) — no more "reserved for future use" scenarios param. `casen synth` now writes a `<file>.bpmn.tests.json` sidecar from a plan's embedded `tests`, consumable by the existing `casen test`.

Along the way, fixed two pre-existing optimizer bugs: `feel/empty-condition` false-positived on the single outgoing flow of an auto-inserted join gateway, and `flow/unreachable` never followed `attachedToRef` so every boundary event (and its downstream path) was always flagged unreachable.

## 2026-07-07 — AI BPMN generation: deep analysis + implementation spec

Research-only change (no code). Added `doc/ai-bpmn-generation-analysis.md` — a deep evaluation of the whole monorepo in the context of AI-driven BPMN generation with Claude Code: current pipeline (compact format → nested `claude` CLI via proxy), package-by-package verdicts, verified Camunda agentic-AI facts (AI Agent Sub-process connector `io.camunda.agenticai:aiagent-job-worker:1`, `fromAi()` tool declaration, no `zeebe:adHocImplementation` attribute — byte-checked against the 116 bundled OOTB templates), a 12-item gap analysis (lossy CompactDiagram IR, `zeebe:property` bindings dropped by `templateToServiceTaskOptions`, no FEEL parse-error lint, no Zeebe-parity deploy rules in the TS linter, no deterministic AI-agent constructor, skill drift/bugs), and a deterministic-vs-non-deterministic responsibility contract. Companion `doc/spec-bpmn-generation-skills.md` is a self-contained implementation handoff: WP0 bugfixes (broken `update-connectors.mjs` path, `lint lint`, `bpmn_deploy` schema drift), WP1 `@bpmnkit/connectors` (catalog search + complete template application incl. `zeebe:property` and inbound templates + generated skill reference), WP2 core builder additions (`aiAgent()` constructor with `fromAi` tooling, `completionCondition`, assignment/schedule, message `correlationKey`), WP3 ProcessPlan IR + `casen synth`/`plan extract`/lossless merge, WP4 deploy-grade lint profile, WP5 honest simulation with scripted agent mocks, WP6 consolidated CLI-first skills v2, WP7 golden-prompt eval harness, WP8 docs.
