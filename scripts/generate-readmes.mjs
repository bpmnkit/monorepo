...
function footer(currentPkg) {
	const packages = [
		...
		},
		{
			...
			desc: "AI task worker — classify, summarize, extract, and decide using Claude",
		},
...
to WebAssembly (experimental). See [Conformance](${DOCS}/getting-started/conformance).
...
See the [Pattern Library guide](https://bpmnkit.com/docs/guides/patterns) for a full walkthrough.
`,
	},
...
See the [Standalone Workers guide](https://bpmnkit.com/docs/guides/workers-standalone) for deployment options (local dev, Docker, Camunda SaaS).
`,
	},
...
| [pnpm](https://pnpm.io/) | 12.4.1 — the version pinned in \`packageManager\`, installed up front |
...
const rootPath = resolve(ROOT, "README.md")
writeFileSync(rootPath, rootReadme, "utf8")
...
console.log(`\nGenerated ${Object.keys(packages).length + 1} READMEs (including root).`)
